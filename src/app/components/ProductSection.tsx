...
import { useCart } from '@/app/contexts/CartContext'; // ✅ ИСПРАВЛЕНО
import { useCurrency } from '@/app/contexts/CurrencyContext'; // ✅ ИСПРАВЛЕНО
import { LazyImage } from './LazyImage';
import { rafThrottle, passiveEventOptions } from '@/lib/performance-utils'; // ✅ ИСПРАВЛЕНО
...
export function ProductSection() {
  ...
  const [visibleCount, setVisibleCount] = useState(3); // Показываем первые 3 сразу (уменьшено с 4)
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3">
          {filters.map((filter) => (
            <button
              key={filter}
              onClick={() => {
                // ✅ Wrap filter change in startTransition to avoid blocking UI
                startTransition(() => {
