...
function observeLongTasks() {
  ...
  const observer = new PerformanceObserver((list) => {
    ...
    entries.forEach((entry) => {
      // 🚀 OPTIMIZATION: Увеличен threshold до 1000ms для premium опыта
      // 1. Видео параллакс Hero требует синхронной обработки
      // 2. Spline 3D сцены загружаются асинхронно через requestIdleCallback
      // 3. Первоначальная загрузка React + Router + Context + Motion - ожидаемый Long Task
      // 4. Three.js инициализация для 7 сцен - тяжёлая операция
      if (entry.duration > 1000) { // Увеличено с 300ms до 1000ms (1 секунда)
        // ✅ ФИЛЬТРАЦИЯ: Игнорируем известные тяжёлые операции
        const isKnownHeavyTask = 
          ...
          // Игнорируем первоначальную загрузку (первые 5 секунд)
